...
def findLocalPath(ref_path, vehicle_longitude, vehicle_latitude):
    ...
    if current_waypoint + 100 > len(ref_path.poses) : # 만약 가장 가까운 waypoint에서 +50번째에 있는 waypoint를 찾고자 했으나 전체 global_path 인덱스를 초과한다면 도착점에 가까워진 것이므로 global_path의 마지막 waypoint를 local_path의 종점으로 활용
